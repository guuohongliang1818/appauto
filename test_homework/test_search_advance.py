...
class TestSearchAdvance:
    ...
    @pytest.mark.parametrize("select_type", [1, 2, 3], ids=["话题/帖子", "类别/标签", "用户"])
    @pytest.mark.parametrize("top_status_type", range(1, 6))
    ...
    @pytest.mark.parametrize("select_type", [1, 2, 3], ids=["话题/帖子", "类别/标签", "用户"])
    @pytest.mark.parametrize("own_tag", range(1, 6))
    ...
    def test_only_back_search(self, only_back_type):
        assert self.search_advance.only_back_search(only_back_type).get_search_result().search_result
    ...
    def test_post_person_search(self, post_person, select_type):
        assert self.search_advance.post_person_search(post_person).select_type_search(
            select_type).get_search_result().search_result
    ...
    def test_date_search(self, select_type, date):
        assert self.search_advance.date_search(date).select_type_search(select_type).get_search_result().search_result
